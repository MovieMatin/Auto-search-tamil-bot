...
API_HASH = environ.get('API_HASH', 'ecd0e2a4cc383ae5717059e7ae120adb')
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5576458964').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL','-1001553347914')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001893213697')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001552763930')
...
DATABASE_NAME = environ.get('DATABASE_NAME', "DF_Movies2")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'Telegram_files')

#stream feature - @TeamHMT_Bots
STREAM_BIN = environ.get("STREAM_BIN", "-1001812984233") #Log channel/Bin Channel id -100xxxxxxx
STREAM_URL = environ.get("STREAM_URL", "streaminglink-2215bd033641") # https://example.com/ (Your Stream Url)
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001812984233'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', '+HLvrlmwiWRpmODc1')
